...
export function PropsDetails({ component }: PropsDetailsProps) {
  const comp = components[component].props
    ? Object.entries(components[component].props!).map(([key, value]) => (
        <Table.Tr key={key}>
          <Table.Td className="font-semibold">{key}</Table.Td>
          <Table.Td>
            <code>{value.value as string}</code>
          </Table.Td>
          <Table.Td>{value.description}</Table.Td>
          <Table.Td>{value.required ? "✅" : "❌"}</Table.Td>
          <Table.Td>
